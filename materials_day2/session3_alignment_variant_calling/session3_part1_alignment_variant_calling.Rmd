...
Once data are in a FASTQ format the second step, after checking quality and trimming bad reads, of any NGS analysis is to align the short reads against the reference genome. 
In this session, we will learn how to map short DNA sequence reads, assess the quality of the alignment and prepare to visualize the mapping of the reads.
...
There are many ways to perform variant calling for short read (illumina) data:
- bcftools
- GATK
- FreeBayes
- Stacks

We will be using GATK in this session
...
Note that this step requires a ‘*truth*’ or ‘*known*’ set of variants. For this example we will be using the gold set from the 1000 genomes project (provided in the sample dataset: 3d7_hb3.combined.final.vcf.gz.tbi). An index for the VCF is required as well and is also provided. If you need to build an index for your VCF file, you can build one easily using the TABIX program, like so:

```{bash}
tabix -p vcf 3d7_hb3.combined.final.vcf.gz
```

## Step 1: Analyze Covaration

```{bash}
java -jar $GATK_JAR -T BaseRecalibrator -R ref/Pf3D7_genome.fasta -I dedup_reads.bam -knownSites 3d7_hb3.combined.final.vcf.gz -o recal_data.table
```
...
You should get two new files: filtered_snps.vcf and filtered_snps.vcf.idx
...
You should get two new files: filtered_indels.vcf and filtered_indels.vcf.idx
...
```{bash}
java -jar software/snpEff/4.1/snpEff.jar -v 3D7 filtered_snps_renamed.vcf > filtered_snps.ann.vcf
...
For example, if you are working with Plasmodium falciparum 3D7:

```{bash}
java -jar /share/apps/snpeff/4.1g/snpEff.jar databases | grep -i 3D7
```

Select the reference/version you are working with (for example: Plasmodium falciparum 3D7), then:

```{bash}
java -jar snpEff.jar download 3D7
```
